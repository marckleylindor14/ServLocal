...
import {
  Upload, X, Loader2, BarChart3, Settings as SettingsIcon,
      ...
      <div className="min-h-screen bg-background text-foreground font-sans">
        ...
        <main className="max-w-2xl mx-auto px-4 py-6 md:py-12 space-y-8">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link
              to="/settings"
              className="flex items-center gap-1.5 text-sm text-primary hover:underline press no-select"
            >
          ...
          <form onSubmit={handleSave} className="space-y-6 bg-card/50 backdrop-blur-md border border-border/40 rounded-2xl p-6">
            ...
            <div className="flex flex-col items-center">
              <div className="relative w-24 h-24 rounded-full overflow-hidden border-2 border-primary">
                ...
                {photoFile && (
                  <button type="button" onClick={clearPhoto} className="absolute top-0 right-0 bg-black/60 text-white p-1 rounded-full press">
                    <X size={14} />
                  </button>
                )}
              </div>
              <button type="button" onClick={() => fileInputRef.current?.click()} className="mt-2 text-sm text-primary hover:underline press">
                Changer la photo
              </button>
            ...
            <button type="submit" disabled={uploading}
              className="w-full bg-primary text-primary-foreground font-semibold py-3 rounded-full hover:bg-primary/90 transition disabled:opacity-50 flex items-center justify-center gap-2 press">
              ...
              <div className="space-y-4">
                ...
                {verificationDocPreview ? (
                  <div className="relative w-full h-48 rounded-lg overflow-hidden">
                    ...
                    <button type="button" onClick={clearVerificationDoc} className="absolute top-2 right-2 bg-black/60 text-white p-1 rounded-full press"><X size={18} /></button>
                  </div>
                ) : (
                  <div onClick={() => verificationFileRef.current?.click()} className="border-2 border-dashed border-border rounded-lg p-6 text-center cursor-pointer hover:border-primary transition text-muted-foreground press">
